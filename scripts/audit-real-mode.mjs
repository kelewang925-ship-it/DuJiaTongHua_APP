...
for (const [relativePath, capability] of requirements) {
  ...
  if (!text.includes(capability) || !/hasCapability/.test(text)) {
    failures.push(`${relativePath}: missing ${capability} Capability guard`);
  }
  if (!text.includes('未开放')) {
    failures.push(`${relativePath}: Real capability must display an explicit 未开放 message`);
  }
}
...
for (const relativePath of appFiles) {
  ...
  for (const rule of forbiddenPagePatterns) {
    if (rule.pattern.test(text)) failures.push(`${relativePath}: ${rule.message}`);
  }
  const usedMockActions = mockWriteActions.filter((name) => new RegExp(`\\b${name}\\b`).test(text));
  if (usedMockActions.length && !/hasCapability|getApiMode/.test(text)) {
    failures.push(`${relativePath}: mock write action lacks a Real/Mock guard (${usedMockActions.join(', ')})`);
  }
}
