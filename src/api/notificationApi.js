...
export async function markNotificationRead(id) {
  ...
  try {
    ...
    const { data, error } = await context.supabase.from('notifications').update({ read_at: readAt }).eq('id', id).eq('user_id', context.user.id).select('*').single();
    return error ? createApiError(error, '标记通知已读失败') : createApiResponse(fromDatabase(data));
  } catch (error) {
...
export async function markAllNotificationsRead() {
  if (isMockMode()) return requestMock({ readAt: new Date().toISOString() });
  try {
    const context = await getAuthenticatedContext();
    const readAt = new Date().toISOString();
    const { error } = await context.supabase.from('notifications').update({ read_at: readAt }).eq('user_id', context.user.id).is('read_at', null);
    return error ? createApiError(error, '全部标记已读失败') : createApiResponse({ readAt });
  } catch (error) {
