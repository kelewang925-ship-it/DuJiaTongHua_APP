        ...
        <FairyCard style={styles.formCard} padding={compact ? spacing.lg : spacing.xl}>
          <View style={styles.heroRow}>
            <View style={styles.heroCopy}>
              <Text style={styles.heroTitle}>纪念日资料</Text>
              <Text style={styles.heroText}>写下名称和日期，之后可以在倒计时页继续准备记录与分享封面。</Text>
            </View>
          ...
          <View style={styles.reminderRow}>
            {reminderOptions.map((days) => {
              const active = days === reminderDays;
              return (
                <Pressable key={days} onPress={() => setReminderDays(days)} style={({ pressed }) => [styles.reminderOption, active && styles.reminderActive, pressed && styles.pressed]}>
                  <Ionicons name="notifications-outline" size={17} color={active ? colors.primaryDeep : colors.textSoft} />
                  <Text style={[styles.reminderText, active && styles.reminderTextActive]}>提前 {days} 天</Text>
                </Pressable>
              );
            })}
          </View>
          ...
          <View style={styles.coverRow}>
            <Pressable onPress={() => setToast({ visible: true, message: '当前阶段使用绘本封面预览，真实上传将在后续业务接入。', tone: 'info' })} style={({ pressed }) => [styles.coverPreview, pressed && styles.pressed]}>
              <Ionicons name="image-outline" size={23} color={colors.accent} />
              <Text style={styles.coverPreviewText}>选择封面</Text>
            </Pressable>
