...
const categoryIcons = {
  日记: 'book-outline',
  照片: 'images-outline',
  纪念日: 'calendar-outline',
  'AI 作品': 'color-wand-outline',
};
...
export default function SearchPage() {
  ...
  const searchableItems = useMemo(() => [
    ...
    ...creations.map((item) => ({
      ...item,
      category: 'AI 作品',
      description: item.resultSummary || [item.source, item.status].filter(Boolean).join(' · '),
      tags: [item.type, item.styleName].filter(Boolean),
          ...
          <>
            ...
            {!keyword.trim() ? (
              ...
              <>
                ...
                {result.length ? <View style={styles.list}>{result.map((item) => <FairyCard key={`${item.category}-${item.id}`} onPress={() => openResult(item)} style={[styles.resultCard, compact && styles.resultCardCompact]} padding={spacing.md} accessibilityRole="button"><View style={[styles.thumb, compact && styles.thumbCompact]}>{isReal ? <View style={[styles.realThumbPlaceholder, { height: compact ? 138 : 156 }]}><Ionicons name={categoryIcons[item.category] || 'document-outline'} size={32} color={colors.textSoft} /><Text style={styles.realThumbText}>{item.category || '记录'}</Text></View> : <FairyImage name={item.imageName} height={compact ? 138 : 156} radius={18} framed={false} resizeMode="cover" />}</View><View style={styles.resultBody}><View style={styles.resultMeta}><FairyTag tone={item.category === 'AI 作品' ? 'default' : 'gold'}>{item.category || '记录'}</FairyTag>{item.date ? <Text style={styles.resultDate}>{item.date}</Text> : null}</View><Text numberOfLines={2} style={styles.resultTitle}>{item.title || '未命名记录'}</Text>{item.description ? <Text numberOfLines={compact ? 2 : 3} style={styles.resultDescription}>{item.description}</Text> : <Text style={styles.missingDescription}>没有可展示的摘要</Text>}<View style={styles.tagRow}>{(item.tags || []).filter(Boolean).slice(0, 3).map((tag) => <View key={tag} style={styles.inlineTag}><Text style={styles.inlineTagText}>{tag}</Text></View>)}</View><View style={styles.openRow}><Text style={styles.openText}>打开记录</Text><Ionicons name="chevron-forward" size={17} color={colors.primaryDeep} /></View></View></FairyCard>)}</View> : <FairyEmptyState imageName="emptySearch" title="没有找到相关内容" description="换个词或分类试试看。" actionTitle="清空搜索" onAction={() => { setKeyword(''); setActiveFilter('全部'); }} />}
              </>
            )}
          </>
        ) : null}
...
const styles = StyleSheet.create({
  ...
  list: { gap: spacing.md }, resultCard: { flexDirection: 'row', gap: spacing.lg, backgroundColor: 'rgba(255,249,244,0.96)' }, resultCardCompact: { flexDirection: 'column' }, thumb: { width: 190 }, thumbCompact: { width: '100%' }, realThumbPlaceholder: { width: '100%', borderRadius: 18, alignItems: 'center', justifyContent: 'center', gap: spacing.sm, backgroundColor: colors.background, borderWidth: 1, borderColor: colors.border }, realThumbText: { color: colors.textSoft, fontSize: 11, fontWeight: '800' }, resultBody: { flex: 1, minWidth: 0, padding: spacing.sm }, resultMeta: { flexDirection: 'row', alignItems: 'center', gap: spacing.sm }, resultDate: { color: colors.textSoft, fontSize: 11 }, resultTitle: { color: colors.text, fontSize: 18, fontWeight: '900', marginTop: spacing.sm }, resultDescription: { color: colors.textSoft, lineHeight: 20, marginTop: spacing.sm }, missingDescription: { color: colors.textSoft, fontSize: 12, fontStyle: 'italic', marginTop: spacing.sm }, tagRow: { flexDirection: 'row', flexWrap: 'wrap', gap: 6, marginTop: spacing.md }, inlineTag: { paddingHorizontal: spacing.sm, paddingVertical: 4, borderRadius: 10, backgroundColor: colors.cardPink }, inlineTagText: { color: colors.accent, fontSize: 10, fontWeight: '800' }, openRow: { flexDirection: 'row', alignItems: 'center', justifyContent: 'flex-end', gap: 4, marginTop: spacing.md }, openText: { color: colors.primaryDeep, fontSize: 12, fontWeight: '900' },
});
