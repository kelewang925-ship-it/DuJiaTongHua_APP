...
export default function ExportPreviewPage() {
  ...
  const counts = useMemo(() => ({ diary: records.filter((item) => item.type === '日记').length, photo: records.filter((item) => item.type === '照片').reduce((sum, item) => sum + (item.photoCount || 0), 0), anniversary: anniversaries.length, ai: creations.length }), [anniversaries.length, creations.length, records]);
  ...
  const pageSamples = [
    ...
    { key: 'diary', label: '日记页', image: 'homeCover', title: records[0]?.title || '日记章节预览' },
  ...
  return (
    <FairyPage backgroundName="creamPaper" header={<FairyHeader showBack title="导出预览" right={<View style={styles.headerActions}><Pressable accessibilityRole="button" onPress={() => canExport ? setToast({ tone: 'info', message: 'Mock 预览分享仅用于流程演示。' }) : showUnavailable()} style={({ pressed }) => [styles.headerButton, pressed && styles.pressed]}><Ionicons name="share-social-outline" size={21} color={colors.text} /></Pressable><Pressable accessibilityRole="button" onPress={confirmExport} style={({ pressed }) => [styles.headerButton, pressed && styles.pressed]}><Ionicons name="download-outline" size={23} color={colors.text} /></Pressable></View>} />} topSpace={28} bottomSpace={60} contentStyle={styles.pageContent} showsVerticalScrollIndicator>
      <View style={styles.content}>
        <View style={styles.intro}><Text style={styles.introTitle}>{canExport ? '翻看这本回忆册的封面与章节' : 'PDF 导出预览尚未开放'}</Text><Text style={styles.introText}>{canExport ? '确认内容、页数和规格后，可体验 Mock 导出流程。' : '当前 Real 模式只展示版式配置，不会生成、下载或分享真实文件。'}</Text></View>
