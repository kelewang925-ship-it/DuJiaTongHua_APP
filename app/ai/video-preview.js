...
export default function VideoPreviewPage() {
  ...
  const video = creations.find((item) => item.id === videoId && item.type === '视频') || null;
  const [playing, setPlaying] = useState(false);
  const [cover, setCover] = useState(coverOptions[0].id);
  const [subtitle, setSubtitle] = useState(video?.subtitle || '');
  const [music, setMusic] = useState(video?.music || musicOptions[0]);
  ...
  const durationLabel = video?.durationLabel || video?.duration || '--:--';
  const showUnavailable = () => setToast({ message: 'Real 模式暂未开放 AI 视频生成、保存与分享。', tone: 'info' });
  const showMissing = () => setToast({ message: '当前没有可操作的视频作品。', tone: 'info' });
  const saveVideo = () => !video ? showMissing() : canGenerate ? setToast({ message: '视频已保存到 Mock 创作历史', tone: 'success' }) : showUnavailable();
  const shareVideo = () => !video ? showMissing() : canGenerate ? setToast({ message: 'Mock 分享卡片已经准备好', tone: 'success' }) : showUnavailable();
  const togglePlaying = () => !video ? showMissing() : canGenerate ? setPlaying((value) => !value) : showUnavailable();

  const headerTitle = !canGenerate ? 'AI 视频未开放' : video ? '视频预览' : '作品不存在';

  return (
    <FairyPage backgroundName="creamPaper" header={<FairyHeader showBack title={headerTitle} right={video ? <Pressable onPress={saveVideo} hitSlop={10}><Ionicons name={canGenerate ? 'save-outline' : 'lock-closed-outline'} size={24} color={colors.text} /></Pressable> : null} />} topSpace={18} bottomSpace={64}>
      <View style={styles.content}>
        {!canGenerate ? (
          <>
            <Text style={styles.subtitle}>当前 Real 模式不会创建或保存模拟视频</Text>
            <FairyCard style={styles.unavailableCard}><Ionicons name="lock-closed-outline" size={25} color={colors.gold} /><Text style={styles.unavailableText}>AI 视频、播放、保存、分享和重新生成均暂未开放。</Text></FairyCard>
          </>
        ) : !video ? (
          <>
            <FairyEmptyState imageName="emptyAiHistory" title="没有找到这份视频" description="请从创作历史打开一条真实存在的视频作品。" />
            <FairyButton title="返回创作历史" onPress={() => router.replace('/ai/history')} leftContent={<Ionicons name="time-outline" size={19} color={colors.white} />} />
          </>
        ) : (
          <>
            ...
            <FairyCard style={styles.playerCard}>
              <View style={styles.player}>
                <FairyImage name={video.previewImageName || activeCover.image} height={360} radius={22} framed={false} />
                <View style={styles.playerShade} />
                <Pressable onPress={togglePlaying} style={({ pressed }) => [styles.playButton, pressed && styles.pressed]}><Ionicons name={playing ? 'pause' : 'play'} size={31} color={colors.brown} /></Pressable>
                <Text style={styles.playerTitle}>{video.title || '未命名视频'}</Text>
                <View style={styles.progressTrack}><View style={[styles.progressFill, { width: playing ? '58%' : '0%' }]} /></View>
                <View style={styles.timeRow}><Text style={styles.timeText}>{playing ? video.currentTimeLabel || '播放中' : '00:00'}</Text><Text style={styles.timeText}>{durationLabel}</Text></View>
              </View>
            </FairyCard>
            <FairyCard style={styles.editCard}>
              <SectionTitle icon="image-outline" title="封面" />
              <View style={styles.coverRow}>{coverOptions.map((item) => { const active = cover === item.id; return <Pressable key={item.id} onPress={() => setCover(item.id)} style={({ pressed }) => [styles.coverOption, active && styles.coverActive, pressed && styles.pressed]}><FairyImage name={item.image} height={116} radius={13} framed={false} /><Text style={styles.coverLabel}>{item.label}</Text>{active ? <View style={styles.check}><Ionicons name="checkmark" size={14} color={colors.white} /></View> : null}</Pressable>; })}</View>
            </FairyCard>
            <FairyCard style={styles.editCard}><SectionTitle icon="chatbubble-ellipses-outline" title="字幕" /><FairyInput value={subtitle} onChangeText={setSubtitle} maxLength={32} placeholder="写下一句片中字幕" containerStyle={styles.noMargin} /></FairyCard>
            <FairyCard style={styles.editCard}>
              <SectionTitle icon="musical-notes-outline" title="音乐" />
              <View style={styles.musicRow}>{musicOptions.map((item) => <Pressable key={item} onPress={() => setMusic(item)} style={({ pressed }) => [styles.musicChip, music === item && styles.musicActive, pressed && styles.pressed]}><Text style={[styles.musicText, music === item && styles.musicTextActive]}>{item}</Text></Pressable>)}</View>
              <View style={styles.songRow}><Ionicons name="musical-note" size={20} color={colors.primaryDeep} /><View style={styles.songCopy}><Text style={styles.songTitle}>{video.musicTitle || '未选择音乐'}</Text><Text style={styles.songTime}>{video.musicDurationLabel || `${music}版`}</Text></View><Ionicons name="play-circle-outline" size={28} color={colors.accent} /></View>
            </FairyCard>
            <FairyCard style={styles.editCard}><SectionTitle icon="film-outline" title="片段" note={clips.length ? '点击可编辑' : '暂无片段数据'} />{clips.length ? <View style={styles.clipRow}>{clips.map((item, index) => <Pressable key={item.id || `${index}`} onPress={() => setToast({ message: `已选中第 ${index + 1} 个片段`, tone: 'info' })} style={({ pressed }) => [styles.clip, pressed && styles.pressed]}><FairyImage name={item.imageName || video.previewImageName || activeCover.image} height={102} radius={12} framed={false} /><View style={styles.clipMeta}><Text style={styles.clipIndex}>{index + 1}</Text><Text style={styles.clipTime}>{item.durationLabel || '--:--'}</Text></View></Pressable>)}</View> : <Text style={styles.emptyClips}>该作品暂未返回可编辑的片段信息。</Text>}</FairyCard>
            <View style={styles.actionRow}><FairyButton title="保存视频" onPress={saveVideo} style={styles.actionButton} leftContent={<Ionicons name="save-outline" size={19} color={colors.white} />} /><FairyButton title="分享给 TA" variant="secondary" onPress={shareVideo} style={styles.actionButton} leftContent={<Ionicons name="heart-outline" size={19} color={colors.text} />} /></View>
