...
- 已将旧版设计稿替换为重新生成的 PNG 效果图，并保留设计索引。
...
当前交付路线统一使用三个开发阶段：第一阶段完成工程与界面基础，第二阶段完成 Supabase 与核心业务真实化，第三阶段完成 AI 真实化与 Free/VIP 权益系统。本文早期使用的 Phase 1–9 是历史拆分，不再作为当前阶段编号；其中组件、API、Storage、Auth 等基础工作已并入第一/二阶段，导出与上线准备统一列为第三阶段之后的后续工作。
...
# 历史基础工作：API 层和 mock/real 模式（已并入第二阶段）
...
# Phase 3：AI 真实化与 Free/VIP 权益系统

## 目标

把童话工坊从模拟入口变成受服务端控制的真实 AI 生成能力，并建立 Free/VIP、魔法点数、任务计费、失败退款和版本化配置。完整实施协议以 `docs/phase-3-ai-module-plan.md` 为准。

## 处理内容

1. 扩展 AI 任务表，增加配置、模型、权益和计费快照。
2. 增加权益、点数钱包、不可变流水、每日额度和版本化配置表。
3. 使用 Edge Functions 安全调用 SiliconFlow，供应商 key 和模型映射只存在服务端。
4. Free 只开放分镜与文本/日记漫画；VIP 开放照片编辑漫画与视频，并通过测试授权验证。
5. 生成结果立即持久化到私有 Supabase Storage，App 只使用短期签名 URL。
6. 服务端原子预扣、结算和技术失败退款；保留 mock 模式。
7. 所有额度、点数、模型、重试和预算使用可回滚的配置版本管理。
...
- `docs/phase-3-ai-module-plan.md`
- `supabase/functions/create-ai-job/index.ts`
- AI 配置、权益、钱包、流水和每日用量迁移文件

## 具体做法

1. App 只提交业务类型、素材、文本和风格，不能提交 provider、model、tier、cost 或配置版本。
2. Edge Function 验证 session、active couple、功能开关、每日测试预算和幂等键。
3. 服务端读取 active 配置，原子占用 Free 额度或 VIP 点数，并创建任务快照。
4. Worker 调用供应商，默认最多系统重试 1 次。
5. 供应商临时结果立即转存到私有 `ai-comics` / `ai-videos` bucket。
6. 成功时结算，技术失败时按任务快照退款并记录点数流水。
7. App 默认每 5 秒轮询，或在后续切换 Realtime；所有状态使用稳定业务错误码。
8. 第三阶段不接真实支付，月卡 ¥18、年卡 ¥168 仅作为后续商品定义，VIP 由有期限的测试授权开启。

## 验收标准

- 至少完成一个 Free 文本漫画真实闭环和一个 VIP 点数真实闭环。
- 并发请求不会超发免费额度或重复扣点，技术失败会原子退款。
- 任务有真实状态变化，结果已落入私有 Storage，不依赖供应商临时 URL。
- 配置切换后，在途任务继续使用创建时的配置和成本快照。
- 三账号 RLS/Storage 越权测试通过，AI Key、模型路由和内部价格不出现在前端。
- mock mode 仍可独立运行。
...
当前先完成第二阶段的 Supabase 远程迁移、三账号 RLS/Storage 验证和双账号真实业务闭环；不要在第二阶段权限与存储基础尚未验收时提前开启真实 AI 调用。

```text
Phase 2：Supabase 与核心业务真实化收口
```

第一条 Codex 任务建议是：

```text
请先读取 docs/resume-state.md、docs/backend-and-api-plan.md 和 docs/phase-3-ai-module-plan.md，以 Git 与远程 Supabase 实际状态为准，完成第二阶段剩余迁移、三账号 RLS/Storage 和双账号真实业务联调。不要提前写入真实 AI key 或开启 AI_REAL_ENABLED。第二阶段验收完成后，再按第三阶段计划创建配置 v1、权益/点数迁移和统一 AI 任务函数。
```
...
- [ ] 第二阶段：配置 Supabase 开发项目并完成远程迁移
- [ ] 第二阶段：完成三账号 RLS/Storage 权限验证
- [ ] 第二阶段：完成双账号 Web/Android 核心业务联调
- [ ] 第二阶段：完成剩余 API、Store 和 Real 页面审计

### Next

- [ ] Phase 3：创建 AI 配置 v1、功能开关和私有结果 buckets
- [ ] Phase 3：实现 Free/VIP 权益、魔法点数和原子结算/退款
- [ ] Phase 3：完成 SiliconFlow 真实任务与 App 闭环
...
《独家童话》APP 当前第一阶段已完成，第二阶段代码为 partial 并等待 Supabase 远程联调；第二阶段验收后进入当前 Phase 3，完成 AI 真实化、Free/VIP 权益、魔法点数和服务端版本化配置，再推进支付、导出与上线。
