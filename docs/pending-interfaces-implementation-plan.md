...
| 术语 | 统一用法 |
| --- | --- |
| `Phase` / `阶段` | 文档标题和路线图中可以保留 `Phase N`，正文统一写作“阶段”。例如：`Phase 5：Supabase 接入阶段`。 |
| `mock MVP` | 表示“不接真实后端、不接真实 AI，只完成本地可演示交互闭环”的页面实现。 |
| `真实能力` | 表示需要接入真实 Supabase、Storage、AI 服务、PDF 导出、系统相册、登录态等生产能力。 |
| `骨架页` | 表示只有说明或静态展示，没有真实 mock 状态和交互闭环的页面。当前路由中骨架页数量为 0。 |
| `兼容路由` | 表示为旧入口、旧设计稿或通知跳转保留的转发页面，不作为新的主开发入口。 |
